		function showPopup(event, image, width, height, text, pos) {
		    ...
		    function Show() {
		        hideWin('winPopup');
		        rect = document.elementFromPoint(event.clientX, event.clientY).getBoundingClientRect();
		        elm = {
		        ...
		        if (elm.left + widthWin > win.right)
		            x = win.right - widthWin - 15;
		        else
		            x = elm.left;

		        if (elm.bottom + height > win.bottom)
		            y = elm.top - height - 15;
		        else
		            y = elm.bottom;

		        if (image != null)
		            document.getElementById("winPopupImage").src = image;
		...
		function showImage(event, menu) {
		    ...
		    if (menu == null)
		        ...
		        img.src = a.attributes.href.value.replace(".html", " Wada.jpg");
		...
		$(document).ready(function () {
		    ...
		    el.click(function () {
		        ...
		        if (checkedElement.is('ul') && visibleElement.is('ul'))
		            visibleElement.stop().animate({
		                'height': 'toggle'
		            }, 500).parent().removeClass('active');

		        if ((checkedElement.is('ul')) && (!checkedElement.is(':visible'))) {
		            checkedElement.stop().animate({
		                'height': 'toggle'
		            }, 500).parent().toggleClass('active');
		            return false;
		        } else
		            $(this).css('background', 'Linen');
